...
# Mint XAUT tokens on Base Sepolia
...
echo "=============================================="
echo "Minting XAUT on Base Sepolia"
echo "=============================================="
...
if [ -n "$IDENTITY_REGISTRY" ]; then
    IS_VERIFIED=$(cast call $IDENTITY_REGISTRY "isVerified(address)" $RECIPIENT --rpc-url $BASE_SEPOLIA_RPC)
...
# Get balance before
BALANCE_BEFORE=$(cast call $XAUT "balanceOf(address)" $RECIPIENT --rpc-url $BASE_SEPOLIA_RPC)
...
# Get balance after
BALANCE_AFTER=$(cast call $XAUT "balanceOf(address)" $RECIPIENT --rpc-url $BASE_SEPOLIA_RPC)
